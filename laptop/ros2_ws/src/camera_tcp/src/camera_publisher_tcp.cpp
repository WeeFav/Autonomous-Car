...
private:
    void receive_loop() {
        while (rclcpp::ok()) {
            uint32_t img_size_net; // receive image size

            ssize_t ret = recv(sock, &img_size_net, sizeof(img_size_net), MSG_WAITALL);
            if (ret == 0) { // server closed
                RCLCPP_WARN(this->get_logger(), "Server closed the connection");
                break;
            }
            else if (ret < 0) {
                RCLCPP_ERROR(this->get_logger(), "Socket recv error");
                continue; // or handle reconnect
            }

            uint32_t img_size = ntohl(img_size_net); // convert from network byte to local byte order
            if (img_size == 0) {
                RCLCPP_WARN(this->get_logger(), "Received empty image size");
                continue;
            }

            
            std::vector<uchar> buf(img_size); // receive image
            size_t bytes_received = 0;
            while (bytes_received < img_size) {
                ssize_t bytes = recv(sock, buf.data() + bytes_received, img_size - bytes_received, 0);
                if (bytes <= 0) {
                    RCLCPP_WARN(this->get_logger(), "Socket closed or error during image recv");
                    break;
                }
            ...
            if (bytes_received < img_size) {
                RCLCPP_WARN(this->get_logger(), "Incomplete image received, skipping frame");
            ...
            if (img.empty()) {
                RCLCPP_WARN(this->get_logger(), "Failed to decode image, skipping frame");
                continue;
            }
            // else {
